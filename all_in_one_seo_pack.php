...
global $aiosp, $aioseop_options, $aioseop_modules, $aioseop_module_list,
	$aiosp_activation, $aioseop_mem_limit, $aioseop_get_pages_start,
	$aioseop_admin_menu;
...
if ( is_array( $aioseop_options )
	...
	&& isset( $aioseop_options['modules']['aiosp_performance_options'] ) ) {
...
if ( ! function_exists( 'aioseop_activate' ) ) {
	...
	/**
	 * Checkes if the plugin was activated
	...
	function aiosp_add_action_links( $actions, $plugin_file ) {
		...
		$action_links = array(
			'settings' 		=> array(
		    	...
		    	'url'   	=> get_admin_url(
					null,
					"admin.php?page=$aioseop_plugin_dirname/aioseop_class.php"
				)
			),
			'forum' 		=> array(
...
if ( ! function_exists( 'aioseop_init_class' ) ) {
	...
	function aioseop_init_class() {
		...
		if ( defined( 'DOING_AJAX' )
			&& !empty( $_POST )
			&& !empty( $_POST['action'] )
			&& ( $_POST['action'] === 'aioseop_ajax_scan_header' ) ) {
