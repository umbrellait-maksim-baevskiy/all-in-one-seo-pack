...
if ( ! function_exists( 'aioseop_load_modules' ) ) {
	...
	function aioseop_load_modules() {
		...
		$aioseop_modules = new All_in_One_SEO_Pack_Module_Manager(
			apply_filters( 'aioseop_module_list', $aioseop_module_list )
		);
		...
		if ( !empty( $aioseop_options ) ) {
			...
			if ( !empty( $aioseop_options['aiosp_archive_noindex'] ) ) {
				$aioseop_options['aiosp_archive_date_noindex']
					= $aioseop_options['aiosp_archive_author_noindex']
					= $aioseop_options['aiosp_archive_noindex'];
				unset( $aioseop_options['aiosp_archive_noindex'] );
				$update_options = true;
			}
			if ( !empty( $aioseop_options['aiosp_archive_title_format'] )
				&& empty( $aioseop_options['aiosp_date_title_format'] ) ) {
				$aioseop_options['aiosp_date_title_format']
					= $aioseop_options['aiosp_archive_title_format'];
...
if ( ! function_exists( 'aioseop_addmycolumns' ) ) {
	...
	function aioseop_addmycolumns() {
		...
		if ( !empty( $aioseop_options)
			&& !empty( $aioseop_options['aiosp_posttypecolumns'] ) ) {
		...
		if( is_array( $aiosp_posttypecolumns )
			&& in_array( $post_type, $aiosp_posttypecolumns ) ) {
	...
	function aioseop_admin_head() {
		echo
			'<script
				...
				src="' . AIOSEOP_PLUGIN_URL . 'js/quickedit_functions.js"
			>
			</script>';
		?>
		...
		<style>
			...
			@media screen and (max-width: 782px) {
				body.wp-admin th.column-seotitle,
				th.column-seodesc,
	...
	function aioseop_handle_ignore_notice() {
		if ( !empty( $_GET ) ) {
		    ...
		    if ( !empty($_GET['aioseop_ignore_notice'] ) ) {
				add_user_meta(
					$user_id,
					'aioseop_ignore_notice',
					$_GET['aioseop_ignore_notice'],
					false
				);
			}
		}
...
if ( ! function_exists( 'aioseop_output_notice' ) ) {
	...
	function aioseop_output_notice( $message, $id = '', $class = "updated fade" ) {
		...
		echo
			"<div{$class}>"
				. wp_kses_post( $message ) . "<br class=clear />
			</div>";
		return true;
	}
}
	...
	function aioseop_output_dismissable_notice( $message, $id = "", $class = "updated fade") {
		...
		if ( !empty( $current_user ) ) {
			...
			$message =
				...
				'</p>
				<p class="alignright">
					<a class="aioseop_dismiss_link" href="' . $url . '">
						Dismiss
					</a>
				</p>';
		}
...
if ( ! function_exists( 'aioseop_ajax_save_meta' ) ) {
	...
	function aioseop_ajax_save_meta() {
		...
		$result = '';
		if ( in_array( $target, Array( 'title', 'description', 'keywords' ) )
		...
		if( $result != '' ):
			...
			$label = "<label id='aioseop_label_{$target}_{$post_id}'></label>
					<span style='width: 20px;display: inline-block;'></span>
					<strong>
						<i>" . __( 'No', 'all-in-one-seo-pack' ) . ' ' . $target . '</i>
					</strong>';
		endif;
		$nonce = wp_create_nonce( "aioseop_meta_{$target}_{$post_id}" );
		$output = '<a
					id="' . $target . 'editlink' . $post_id . '"
					class="aioseop_edit_link" href="javascript:void(0);"'
					. 'onclick=\'aioseop_ajax_edit_meta_form(
						' . $post_id . ',
						"' . $target . '",
						"' . $nonce . '"
					);
				...
				>'
					. '<img
						class="aioseop_edit_button"
						id="aioseop_edit_id"
						src="' . AIOSEOP_PLUGIN_IMAGES_URL . '/cog_edit.png"
					/>
				</a> ' . $label;
		die(
			"jQuery('div#aioseop_" . $target . "_" . $post_id . "').fadeOut('fast',
			function() { var my_label = " . json_encode( $output ) . ";
...
if ( ! function_exists( 'aioseop_ajax_init' ) ) {
	...
	function aioseop_ajax_init() {
		if ( !empty( $_POST )
			...
			&& (!empty( $_POST['nonce-aioseop'] )
			|| ( !empty( $_POST['nonce-aioseop-edit'] ) ) )
			&& !empty( $_POST['options'] ) ) {
			$_POST = stripslashes_deep( $_POST );
			$settings = esc_attr( $_POST['settings'] );
			if ( ! defined( 'AIOSEOP_AJAX_MSG_TMPL' ) )
			    define(
					'AIOSEOP_AJAX_MSG_TMPL',
					"jQuery( 'div#aiosp_$settings' ).fadeOut(
						'fast',
						function(){
							jQuery( 'div#aiosp_$settings' ).html( '%s' ).fadeIn( 'fast' );
						}
					);"
				);
			if ( ! wp_verify_nonce( $_POST['nonce-aioseop'], 'aioseop-nonce' ) )
				die( sprintf(
					AIOSEOP_AJAX_MSG_TMPL,
					__( "Unauthorized access; try reloading the page.", 'all-in-one-seo-pack' )
				) );
		} else {
			die(0);
		}
...
if ( ! function_exists( 'aioseop_ajax_delete_url' ) ) {
	...
	function aioseop_ajax_delete_url() {
		...
		if ( !empty( $_POST['aiosp_sitemap_addl_pages'] )
			&& ( !empty( $_POST['aiosp_sitemap_addl_pages'][ $options ] ) ) ) {
			...
			$output = sprintf(
				__( "Row %s not found; no rows were deleted.", 'all-in-one-seo-pack' ),
				esc_attr( $options )
			);
		}
		die( sprintf( AIOSEOP_AJAX_MSG_TMPL, $output ) );
...
if ( ! function_exists( 'aioseop_ajax_scan_header' ) ) {
	...
	function aioseop_ajax_scan_header() {
		...
		if ( empty( $meta ) )
			$meta = '<span style="color:green;">'
						. __( 'No duplicate meta tags found.', 'all-in-one-seo-pack' ) .
					'</span>';
		else {
			$meta = "<table
						cellspacing=0
					...
					"</table>";
			$meta .= "<p>
						<div class='aioseop_meta_info'>
							<h3 style='padding:5px;margin-bottom:0px;'>"
								. __( 'What Does This Mean?', 'all-in-one-seo-pack' ) .
							"</h3>
							<div style='padding:5px;padding-top:0px;'>"	.
								"<p>"
									. __(
										'All in One SEO Pack has detected that
											a plugin(s) or theme is also outputting
											social meta tags on your site.  
											You can view this social meta in the source
											code of your site (check your browser
											help for instructions
											on how to view source code
										).',
										'all-in-one-seo-pack'
									) .
								"</p>
								<p>"
									. __(
										'You may prefer to use the social
											meta tags that are being output by
											the other plugin(s) or theme.  
											If so, then you should deactivate
											this Social Meta feature in
								...
								"</p>
									<p>"
										. __(
											'You should avoid duplicate social
												meta tags.  You can use these
												free tools from Facebook, Google
												and Twitter to validate your social
												meta and check for errors:',
											'all-in-one-seo-pack'
										) .
									"</p>";
			...
			$meta .= "<p>" .
						__(
							'Please refer to the document for each tool
								for help in using these to debug your social meta.',
							'all-in-one-seo-pack'
						) .
					"</div>
				</div>";
...
if ( ! function_exists( 'aioseop_ajax_save_settings' ) ) {
	...
	function aioseop_ajax_save_settings() {
		...
		if( AIOSEOPPRO ){
			$output = '<div id="aioseop_settings_header">
							<div id="message" class="updated fade">
								<p>' . $output . '</p>
							</div>
						</div>
						<style>
							body.all-in-one-seo_page_all-in-one-seo-pack-pro-aioseop_feature_manager
				...
				$output = '<div id="aioseop_settings_header">
							...
							<style>
								body.all-in-one-seo_page_all-in-one-seo-pack-aioseop_feature_manager
								.aioseop_settings_left {
									margin-top: 45px !important;
								}
							</style>';
		...
		if ( !empty( $links ) )
			foreach( $links as $k => $v ) {
				if ( $k == "Feature Manager" )
					...
					$current = '';
				$output .= "<li{$current}>
								<a href='" . esc_url($v) . "'>" . esc_attr( $k ) . "</a>
							</li>";
			}
		$output .= "</ul>";
		die( sprintf(
			"jQuery( '{$_POST['target']}' ).fadeOut( 'fast',
			function() { jQuery( '{$_POST['target'] }' ).html('%s').fadeIn( 'fast' ); } );",
			addslashes( $output )
			)
		);
	}
		...
		if( current_user_can( 'edit_post', $id ) ) { ?>
			<div class="aioseop_mpc_admin_meta_container">
				<div
					...
					style="float:left;"
				>
					<?php $content = strip_tags( stripslashes( get_post_meta(
						$id,
						"_aioseop_" .
						$target,
						TRUE ) )
					...
					if( !empty($content) ):
						...
						$label = 	"<label id='aioseop_label_{$target}_{$id}'></label>
										<span style='width: 20px;display: inline-block;'></span>
										<strong>
											<i>" .
												__(
						...
						echo
							'<a
								id="' . $target . 'editlink' . $id . '"
								class="aioseop_edit_link"
...
if ( ! function_exists( 'aioseop_get_pages' ) ) {
	...
	function aioseop_get_pages( $pages ) {
		...
		foreach ( $pages as $k => $v ) {
			$postID = $v->ID;
			$menulabel = stripslashes( get_post_meta(
				$postID,
				'_aioseop_menulabel',
				true )
			);
...
if ( ! function_exists( 'aioseop_list_pages' ) ) {
	...
	function aioseop_list_pages( $content ) {
		...
		$matches = array();
		if ( preg_match_all(
			'/<li class="page_item page-item-(\d+)/i', $content, $matches ) ) {
...
if ( ! function_exists( 'aioseop_filter_callback' ) ) {
	...
	function aioseop_filter_callback( $matches ) {
		...
		return '<li class="page_item page-item-'.$postID.$matches[2].'" >
					<a href="'.$matches[3].'"'.$title_attrib.'>';
	}
}
...
if ( ! function_exists( 'parse_ini_string' ) ) {
	...
	function parse_ini_string( $string, $process_sections ) {
		...
		return parse_ini_file(
			"php://filter/read=parse_ini/resource=php://memory",
			$process_sections
		);
	}
}
...
/**
 * Updates users visibility notices.
 * @since 1.0.0
 */
function aioseop_update_user_visibilitynotice(){
	update_user_meta(
		get_current_user_id(),
		'aioseop_visibility_notice_dismissed',
		true
	);
}

/**
 * Updates yst detected notices.
 * @since 1.0.0
 */
